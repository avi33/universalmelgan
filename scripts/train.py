...
def main():
    ...
    # Load PyTorch Models #
    #######################
    netG = Generator(args['fft']['n_mel_channels'], args['generator']['ngf'], args['generator']['n_residual_layers']).cuda()
    if args['Generator']['G_path'] is not None:
        netG.load_state_dict(torch.load(args['Generator']['G_path'] / "netG.pt"))        
    netD = Discriminator(
        args['Discriminator']['num_D'], args['Discriminator']['ndf'], args['Discriminator']['n_layers_D'], args['Discriminator']['downsamp_factor']
    ).cuda()
    ...
    for epoch in range(1, args['train']['epochs'] + 1):
        for iterno, x_t in enumerate(train_loader):
            ...
            netG.zero_grad()
            (loss_G + args['losses']['labmda_sc']*sc + args['losses']['labmda_sm']*sm).backward()
            print(sc, sm)
            optG.step()
